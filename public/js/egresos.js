...
function inicializarTablaDesdeLocalStorage() {
    ...
    const datosTabla = JSON.parse(localStorage.getItem('datosOtrosEgresos')) || [];
...
function actualizarLocalStorage() {
    ...
    localStorage.setItem('datosOtrosEgresos', JSON.stringify(datosTabla));
...
function addIncomeRow() {
    ...
    const descriptionInput = document.createElement('input');
    descriptionInput.type = 'text';
    descriptionInput.placeholder = 'Descripción del Egreso';
    descriptionInput.style.width = '150px';
    descriptionInput.required = true;
    descriptionInput.addEventListener('input', () => actualizarLocalStorage());
    cellLabel.appendChild(descriptionInput);
...
function renderTable(data) {
    ...
    // Obtener el valor almacenado con la llave 'porcentajeCompra'
    let porcentajeGuardado = localStorage.getItem('porcentajeCompra') || 70;

    // Verificar si existe en localStorage
    if (porcentajeGuardado !== null) {
        console.log(`El porcentaje guardado es: ${porcentajeGuardado}%`);
        // Puedes convertirlo a número si necesitas
        porcentajeGuardado = parseFloat(porcentajeGuardado);
    } else {
        console.log('No hay porcentaje guardado en localStorage.');
    }

    // Renderizar fila de pronósticos
    forecastRow.innerHTML = "<td> Compras</td>";
    Object.values(data).forEach(sale => {
        const td = document.createElement("td");

        // Multiplicar el valor del pronóstico por el porcentaje
        let valorCalculado = !isNaN(sale) ? sale * (porcentajeGuardado /100): NaN;

        td.textContent = !isNaN(valorCalculado) ? valorCalculado.toFixed(2) : "N/A";
        td.dataset.value = valorCalculado;
...
function updateCashRow() {
    const forecastRow = document.querySelector(".income-table tbody tr:nth-child(1)"); // Compras 
    const cashRow = document.querySelector(".income-table tbody tr:nth-child(2)"); // Compras en efectivo
    ...
    if (!cashInput) {
        console.error("No se encontró el input de porcentaje en la fila 'Compras en efectivo'.");
        return;
    }
    ...
    if (!cashInput2) {
        console.error("No se encontró el input de porcentaje en la fila 'Compras vencimiento 1 mes'.");
        return;
    }
    ...
    if (!cashInput3) {
        console.error("No se encontró el input de porcentaje en la fila 'Compras vencimineto a dos meses'.");
        return;
    }
    ...
    let datosTabla = JSON.parse(localStorage.getItem('datosTablaCompras')) || [];
    let datosOtrosIngresos = JSON.parse(localStorage.getItem('datosOtrosEgresos')) || [];

    // Verificar si hay datos
    if (!Array.isArray(datosTabla) || datosTabla.length === 0) {
        console.warn('No hay datos válidos en localStorage para "datosTablaCompras".');
    }

    if (!Array.isArray(datosOtrosIngresos)) {
        console.warn('No hay datos válidos en localStorage para "datosOtrosEgresos".');
        datosOtrosIngresos = [];
    }
    ...
    localStorage.setItem('totalesTablaCompra', JSON.stringify(totales));
}
...
function guardarDatosEnLocalStorage() {
    ...
    let filas = document.querySelectorAll("table tbody tr.CE, table tbody tr.CV1M, table tbody tr.VCV2M");
    ...
    localStorage.setItem('datosTablaCompras', datosJSON);
...
// Función para guardar el porcentaje en localStorage
function guardarPorcentaje() {
    const inputPorcentaje = document.querySelector('#porcentaje');
    const porcentaje = inputPorcentaje.value; // Obtener el valor del input

    if (porcentaje >= 0 && porcentaje <= 100) {
        localStorage.setItem('porcentajeCompra', porcentaje);
...
// Escuchar el evento "input" o "change" para guardar automáticamente
document.querySelector('#porcentaje').addEventListener('change', guardarPorcentaje);
...
function guardarPorcentaje() {
    ...
    if (porcentaje >= 0 && porcentaje <= 100) {
        localStorage.setItem('porcentajeCompra', porcentaje);
        console.log('Porcentaje guardado correctamente en localStorage.');
        actualizarPredicciones(); // Llamar a la función de actualización
    } else {
        alert('Por favor, ingrese un valor válido entre 0 y 100.');
    }
}

// Escuchar el evento "change" para guardar automáticamente
document.querySelector('#porcentaje').addEventListener('change', guardarPorcentaje);

// Función para actualizar la tabla automáticamente
function actualizarPredicciones() {
    console.log('Actualizando predicciones...');
    loadPredictions(); // Cargar y renderizar la tabla con los datos más recientes
    
}

// Escuchar el evento "storage" para cambios en otras pestañas (si es necesario)
window.addEventListener('storage', (event) => {
    if (event.key === 'porcentajeCompra') {
        console.log('Cambio detectado en porcentajeCompra desde otra pestaña.');
        actualizarPredicciones(); // Actualizar en la misma pestaña si cambia en otra
    }
});
